...
import './styles.css';
import { investmentView, valuationFresh } from './valuation.js';
...
function App() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (next.schemaVersion !== 3 || !Array.isArray(next.candidates) || !Array.isArray(next.factors) || !next.disclosures || !next.quality || !Number.isFinite(Date.parse(next.generatedAt))) throw new Error('快照格式不兼容');
  ...
  const filtered = all.filter(c => `${c.name} ${c.symbol} ${c.theme}`.toLowerCase().includes(search.toLowerCase())).filter(c => {
    ...
    return !active(c) || ['avoid', 'extended'].includes(c.status) || ['expensive', 'quality'].includes(decision.key);
  })
