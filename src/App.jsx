...
function Candidate({ item, active, onSelect }) {
  const state = active ? item.status : 'blocked';
  const p = active ? item.plan : null;
...
function Detail({ item, active, onClose }) {
  ...
  return <dialog ref={dialog} className="detail" onCancel={onClose} onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
    ...
    {active && item.blockers.length > 0 && <><h3>还缺什么</h3><ul>{item.blockers.map(x => <li key={x}>{x}</li>)}</ul></>}
    ...
    <p>本筛选未接入一致预期、财报事件日历、实时盘口或全市场资金流，不能判断突发事件是否已被定价。先在交易终端复核，再决定是否执行。</p>
...
function App() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (next.schemaVersion !== 2 || !Array.isArray(next.candidates) || !Array.isArray(next.factors) || !next.disclosures || !next.quality || !Number.isFinite(Date.parse(next.generatedAt))) throw new Error('快照格式不兼容');
  ...
  const filtered = all.filter(c => `${c.name} ${c.symbol} ${c.theme}`.toLowerCase().includes(search.toLowerCase())).filter(c => filter === 'all' || (filter === 'plans' ? active(c) && ['ready', 'watch'].includes(c.status) && c.plan : !active(c) || ['avoid', 'extended'].includes(c.status)))
  ...
  return <main className="shell">
    ...
    <section className="workspace">
      <div className="section-heading"><div><h2>标的与执行条件</h2><p>顺势突破 · 未回测规则筛选 · 评分不代表胜率</p></div><span className="counter">{all.filter(c => active(c) && c.status === 'ready').length} 优先观察 / {all.length} 候选</span></div>
      <div className="toolbar"><div className="tabs" role="tablist" aria-label="选择市场">{Object.entries(MARKETS).map(([key, label]) => <button key={key} role="tab" aria-selected={market === key} onClick={() => { setMarket(key); setSearch(''); }}>{label}</button>)}</div><label className="search"><Search size={16} /><input aria-label="搜索代码或名称" placeholder="代码、名称或板块" value={search} onChange={e => setSearch(e.target.value)} /></label><select aria-label="筛选状态" value={filter} onChange={e => setFilter(e.target.value)}><option value="all">全部状态</option><option value="plans">有条件计划</option><option value="risks">回避 / 暂停</option></select></div>
      {all.every(c => !active(c) || c.status !== 'ready') && <p className="market-note">当前没有同时满足全部条件的优先标的。等待确认或保持观察，也是本次筛选的有效结论。</p>}
      <div className="candidate-grid">{filtered.map(c => <Candidate key={c.key} item={c} active={active(c)} onSelect={setSelected} />)}</div>
      {!filtered.length && <p className="empty">没有符合当前筛选条件的标的。</p>}
    </section>
    <section className="factor-section"><div className="section-heading"><div><h2>影响仓位的宏观价格</h2><p>按观测日期展示，利率变化以基点计；不直接触发买卖</p></div></div>{factors.length ? <div className="factor-grid">{factors.map(f => <article key={f.key} className="factor"><h3>{f.name}</h3><strong>{fmt(f.value)}<small>{f.unit}</small></strong><span>{finite(f.change5obsBp) ? `较5个有效观测前 ${fmt(f.change5obsBp)} bp` : '期权隐含波动率'}</span><p>{f.meaning}</p><small>关联：{f.exposure}</small><a href={safeUrl(f.sourceUrl)} target="_blank" rel="noreferrer">{f.quoteDate} · {f.source} <ExternalLink size={12} /></a></article>)}</div> : <p className="empty">暂无时效窗口内的宏观价格，未展示旧数值。</p>}</section>
    ...
    <details className="quality"><summary>数据状态与覆盖边界 · {data.quality.issues.length} 项采集缺口</summary><p>{data.strategy}</p><p>仅筛选16个预设标的，代表不同敞口，不是全市场选股或基本面评级。黄金、美债ETF同样使用美股基准门槛，防守阶段不会自动推荐其对冲。</p><p>有效行情按交易地收盘和周末检查；法定假期未接入专用日历，宁可保守暂停。BTC只用完整UTC日线，最多容忍收盘后36小时；整个快照14小时失效。</p><p>近期公告：{data.disclosures.failedQueries ? `${data.disclosures.failedQueries}组查询失败，覆盖不完整` : `${news.length}条符合当前窗口`}。美国官员法定披露仅保留官方检索入口，未接入可核验的近期交易记录。</p><p>原始文档中的另类数据、期权大单和一致预期尚无有效数据接入，当前页面不将普通涨跌包装为这些信号。</p>{data.quality.issues.length > 0 && <ul>{data.quality.issues.map((x, i) => <li key={i}>{x.name}：{x.reason}</li>)}</ul>}<p><a href="https://disclosures-clerk.house.gov/FinancialDisclosure" target="_blank" rel="noreferrer">美国众议院披露入口</a> · <a href="https://efdsearch.senate.gov/search/" target="_blank" rel="noreferrer">美国参议院披露入口</a></p></details>
    <footer>日线计划需在下单前复核实时价格和事件风险。未回测，非自动交易；止损参考不是成交或最大损失保证。<a href="/help/market-board-guide.html">完整使用指导 <CircleHelp size={14} /></a></footer>
    {selectedItem && <Detail key={selectedItem.key} item={selectedItem} active={active(selectedItem)} onClose={() => setSelected(null)} />}
  </main>;
}
